...
import 'package:flapjack_client_search/src/api/search_client.dart';
import 'package:flapjack_client_search/src/model/api_key.dart';
import 'package:flapjack_client_search/src/model/get_api_key_response.dart';
  ...
  Future<void> waitKeyCreation({
    ...
    await _waitUntil(
      retry: () async {
        ...
        } on FlapjackApiException catch (_) {
          return null;
        }
      },
  ...
  Future<void> waitKeyDeletion({
    ...
    await _waitUntil(
      ...
      retry: () async {
        try {
          return await getApiKey(key: key, requestOptions: requestOptions);
        } on FlapjackApiException catch (e) {
          return e;
        }
      },
      until: (result) =>
          result is FlapjackApiException ? result.statusCode == 404 : false,
    );
  }
