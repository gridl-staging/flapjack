...
class FlapjackSearchE2eTest < Minitest::Test
  ...
  # Wait for task completion using the SDK's built-in wait method
  ...
  # =========================================================================
  # Cleanup (runs after all tests, regardless of test ordering)
  # =========================================================================
  ...
  Minitest.after_run do
    begin
      hosts = [
        Flapjack::Transport::StatefulHost.new(
          ENV["FLAPJACK_SERVER"] || "localhost",
          protocol: "http://",
          port: (ENV["FLAPJACK_PORT"] || "7700").to_i,
          accept: CallType::READ | CallType::WRITE
        )
      ]
      config = Flapjack::Configuration.new("test-app", "test-api-key", hosts, "Search")
      c = Flapjack::SearchClient.create_with_config(config)
