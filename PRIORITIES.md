...
All launch blockers closed. v1.0.0 released. Debbie sync pipeline wave 2 and wave 3 are complete for the last published lineage (staging/prod at `6166055` / `b7841a0`). Current dev `main` has advanced further with the full Mar 31 hardening stack, Apr 8 dashboard/server cleanup, Apr 15 analytics retention/rollup foundation work, and Apr 15 test-hygiene/query-safety work. The next priority is publishing that newer mainline truth, then closing the design-gated follow-through gaps for search HA ownership/freshness and durable analytics rollup writer/query-planner integration.
...
4. ~~**Experiment handler merge guardrails**~~ — ✅ Resolved (2026-03-31). `/2/abtests/{id}/results` now routes through the shared store-and-ID resolver seam, direct results-endpoint proofs cover store-unavailable plus numeric/UUID resolution, and experiment OpenAPI docs now declare resolver-driven `500` paths consistently. Landed on dev `main` after wave 3 and now needs the next debbie sync.
5. ~~**Runbook parity + admin-key truth sync**~~ — ✅ Resolved (2026-03-31). Standardized `flapjack --data-dir <path> reset-admin-key` across startup output, dashboard auth help, internal auth docs, and operator runbooks, including shell-safe quoting for spaced data-dir paths. Landed on dev `main` after wave 3 and now needs the next debbie sync.
6. ~~**Security baseline follow-through**~~ — ✅ Resolved (2026-03-31). Closed the scoped HTTP-hardening proof gaps by adding invalid-key non-consumption and `FLAPJACK_MAX_BODY_MB` body-limit `413` tests, aligning the security docs/audit, refreshing the committed OpenAPI export, and tightening helper-script safety around sync destinations. Landed on dev `main` after wave 3 and now needs the next debbie sync.
7. ~~**Security baseline docs + test coverage**~~ — ✅ Resolved (2026-03-31). `SECURITY_BASELINE.md` and the audit matrix now capture the shipped CORS/body-limit/trusted-proxy/rate-limit surface with focused proof references, and `startup_catchup.rs` now treats write-queue timeout as warn-and-continue outside strict bootstrap. Landed on dev `main` after wave 3 and now needs the next debbie sync.
8. ~~**Nightly CI + sync hygiene**~~ — ✅ Resolved (2026-03-31). Restored nightly Rust CI parity by creating the dashboard dist stub, added `CHANGELOG.md` / `CONTRIBUTING.md` / `SECURITY.md` to the public sync whitelist, and clarified README vector/hybrid support caveats by binary target. Landed on dev `main` after wave 3 and now needs the next debbie sync.
9. ~~**Operations runbook hardening**~~ — ✅ Resolved (2026-03-31). `OPERATIONS.md` now carries proof-backed startup/readiness/replication/admin-key/snapshot failure runbooks, stronger ownership links to deployment/security/config docs, verified proof citations, and the corrected `flapjack --data-dir <path> reset-admin-key` syntax. Landed on dev `main` after wave 3 and now needs the next debbie sync.
...
1. **Next public sync wave** — publish current dev `main`, now including the Mar 31 pm1-pm6 hardening stack plus Apr 8 and Apr 15 work, to staging/prod.
2. **Search HA ownership/freshness design gate** — do not implement automatic write promotion until index owner, generation/term, replica freshness, restart recovery, and split-brain behavior have one tested source of truth. Safe forwarding/503 behavior is the likely first implementation step after the design gate.
3. **Durable analytics rollup writer/query planner** — build on the merged design/schema/config/manifest foundation, but require known-answer query tests and certified-coverage retention gates before any rollup reads or raw deletion.
4. **OpenAPI snapshot follow-through** — the test-hygiene session handoff called out an OpenAPI export mismatch as deferred; rerun the focused test at current `main` and regenerate `engine/docs2/openapi.json` if still red.
5. **Runbooks iteration** — keep refining `OPERATIONS.md` from real incidents; the docs and admin-key recovery contract now exist, so the remaining work is operational polish rather than missing baseline coverage.

## Post-Launch

See [`ROADMAP.md#open--not-yet-implemented`](ROADMAP.md#open--not-yet-implemented) for post-launch work (next public sync wave, search HA design, durable analytics rollups, runbooks, mobile dashboard, OWASP deep pass).
