...
**Last updated: 2026-04-15 (wave 3 remains the latest published lineage; dev main now also includes the Apr 8 cleanup follow-through plus the Apr 15 analytics-retention and test-hygiene/safety merges, all awaiting the next public sync wave)**
...
| Nightly CI + sync hygiene | ✅ Done | mar31_pm_1 | Restored nightly Rust CI parity by stubbing the dashboard dist asset, added `CHANGELOG.md`/`CONTRIBUTING.md`/`SECURITY.md` to the public sync whitelist, and clarified root README vector/hybrid platform caveats by target. This landed after wave 3 and still needs the next debbie sync to reach staging/prod. |
| Operations runbook hardening | ✅ Done | mar31_pm_2 | `engine/docs2/3_IMPLEMENTATION/OPERATIONS.md` now carries proof-backed startup/readiness/replication/admin-key/snapshot failure runbooks, stronger ownership links to deployment/security/config docs, corrected `flapjack --data-dir <path> reset-admin-key` syntax, and tightened proof citations. This landed after wave 3 and still needs the next debbie sync to reach staging/prod. |
| Security baseline docs + test coverage | ✅ Done | mar31_pm_3 | `engine/docs2/3_IMPLEMENTATION/SECURITY_BASELINE.md` and `engine/docs2/4_EVIDENCE/SECURITY_BASELINE_AUDIT.md` now capture the shipped HTTP hardening surface (CORS, body limits, trusted proxies, per-key rate limiting) with focused proof references, and non-strict startup catchup now warns-and-continues on write-queue timeout. This landed after wave 3 and still needs the next debbie sync to reach staging/prod. |
| Security baseline follow-through | ✅ Done | mar31_pm_4 | Closed the scoped HTTP-hardening proof gaps with invalid-key non-consumption and `FLAPJACK_MAX_BODY_MB` `413` tests, aligned the security docs/audit, refreshed the committed OpenAPI export, and tightened helper-script safety around sync destinations. This landed after wave 3 and still needs the next debbie sync to reach staging/prod. |
| Runbook parity + admin-key truth sync | ✅ Done | mar31_pm_5 | Standardized `flapjack --data-dir <path> reset-admin-key` across startup output, dashboard auth help, `engine/docs/AUTH_DESIGN.md`, and `OPERATIONS.md`, including shell-safe quoting for spaced paths. This landed after wave 3 and still needs the next debbie sync to reach staging/prod. |
| Experiment handler merge guardrails | ✅ Done | mar31_pm_6 | Routed `/2/abtests/{id}/results` through the shared resolver seam, added direct results-endpoint proof for store-unavailable plus numeric/UUID resolution, and aligned experiment OpenAPI `500` docs with resolver behavior. This landed after wave 3 and still needs the next debbie sync to reach staging/prod. |
| Targeted cleanup follow-through | ✅ Done | apr08 | Extracted dashboard experiment normalization/results typing into `engine/dashboard/src/lib/experiment-normalization.ts` and removed a stale server cognitive-complexity suppression. This landed after wave 3 and still needs the next debbie sync to reach staging/prod. |
...
| Durable analytics rollup storage | 🔶 Foundation only | Design note, known-answer query contracts, rollup schema/config helpers, and `RollupManifest` persistence/coverage logic are implemented. No rollup writer, rollup-backed query planner, or certified-coverage retention deletion is wired yet. |
...
The substantive Tier 1 and Tier 2 engineering work is complete, and the OSS launch gate is now closed by staging run `23671792399`. The Stage 3 proof pack exists locally with 2h mixed/write soak artifacts, restart-during-active-writes proofs, and nontrivial crash/restart durability proofs. The former threshold-breach issue (soak runs exiting `99`) was resolved by introducing `SOAK_WRITE_THRESHOLDS` that correctly distinguish sustained-overload acceptance from short-baseline failure detection. Debbie sync wave 3 has published the last public lineage to staging and prod, and the HA boundary/runbook truth-sync is merged. Current dev `main` has advanced further with the full post-wave-3 Mar 31 stack, Apr 8 cleanup, Apr 15 analytics-retention/rollup-foundation work, and Apr 15 test-hygiene/safety work. Those newest doc/CI/security/testing surfaces still need the next debbie sync wave to reach the public clones. See [`engine/loadtest/BENCHMARKS.md`](../loadtest/BENCHMARKS.md) for details. Use [`1_STRATEGY/HIGHEST_PRIORITY.md`](1_STRATEGY/HIGHEST_PRIORITY.md) for the live priority narrative.
...
| PR-12 | Runbooks & incident response | 🟡 In progress (2026-03-31) | Canonical operator docs now live in [`engine/docs2/3_IMPLEMENTATION/OPERATIONS.md`](3_IMPLEMENTATION/OPERATIONS.md) with proof-backed startup/readiness/disk-memory/snapshot/replication/admin-key failure scenarios, observability guarantees, upgrade smoke, rollback guidance, and the explicit `flapjack --data-dir <path> reset-admin-key` recovery contract. Remaining work is real-incident refinement plus any future cleanup of the pre-existing proof-table vs runbook-summary asymmetry. |
...
| PR-15 | Durable analytics rollup writer/query planner | 🔴 Not started / design foundation ready | The Apr 15 merge added the design note, known-answer contracts, rollup schema/config helpers, and manifest coverage logic. The writer, planner, HLL serialization choice, and certified-retention gating are still open. |

### Recommended Execution Order

All Tier 1 and Tier 2 engineering tracks are complete and the OSS launch gate is closed. v1.0.0 released with all 5 binary targets + Docker image. The latest dev-main post-launch hardening now includes the public doc sync contract plus validator coverage, a focused regression-gate follow-through, a merged HA boundary/runbook truth-sync pass, a completed Debbie sync wave 3 that published those surfaces to staging and prod, the Mar 31 pm1-pm6 stack, Apr 8 cleanup, and the Apr 15 analytics/test-hygiene merges. Current post-launch focus is publishing that latest hardening, then closing the explicit design-gated tracks: search HA ownership/freshness, durable analytics rollup writer/query-planner integration, and runbook/security refinement from real incidents.
...
2. **Next public sync wave** — publish current dev `main`, now including the Mar 31 stack, Apr 8 cleanup, and Apr 15 analytics/test-hygiene work.
3. **PR-14: Search HA ownership/freshness design** — unblock safe read fallback/write forwarding decisions before any automatic write promotion.
4. **PR-15: Durable analytics rollup writer/query planner** — build on the merged manifest/schema foundation without risking double-counted analytics or premature raw-data deletion.
5. **PR-12: Runbooks & incident response** — continue from real incidents plus any remaining proof-summary cleanup.
6. **PR-13: Mobile / responsive dashboard** — low priority, desktop-first acceptable.
...
| 2026-03-31 | mar31_pm_1: Nightly CI + sync hygiene | Restored nightly Rust CI parity by stubbing the dashboard dist asset, added `CHANGELOG.md` / `CONTRIBUTING.md` / `SECURITY.md` to `.debbie.toml`, and tightened root README vector/hybrid caveats so pre-built Linux/Windows binary limits match the real release matrix. This landed after wave 3 and is waiting for the next debbie sync. Branch: `batman/mar31_pm_1_ci_and_sync_hygiene`. |
| 2026-03-31 | mar31_pm_2: Operations runbook hardening | Expanded `engine/docs2/3_IMPLEMENTATION/OPERATIONS.md` with proof-backed failure-mode runbooks, ownership cross-links, corrected `reset-admin-key` CLI ordering, and tighter readiness/replication proof citations. This landed after wave 3 and is waiting for the next debbie sync. Branch: `batman/mar31_pm_2_operations_runbook_hardening`. |
...
| 2026-04-15 | apr07_pm_2: analytics retention hardening | Hardened partition-retention behavior with deterministic cutoff tests, preserved the 90-day default, documented the actual env/config surface, added durable rollup design and known-answer query contracts, and merged rollup schema/config/manifest foundation. Rollup writer/query-planner/retention gating remain open. Branch: `batman/apr07_pm_2_analytics_phase5_retention`. |
