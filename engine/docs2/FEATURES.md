...
**Last updated: 2026-03-29 (OSS launch gate closed; v1.0.0 cut; OSS policy docs shipped; OpenTelemetry shipped; TODO stubs replaced with real docs; codebase quality round 2 complete)**
...
- **Load & Stress Testing:** k6 suite in `engine/loadtest/` — smoke, search throughput, write throughput, mixed workload, spike, memory-pressure, plus the long-running `mixed-soak` / `write-soak` scenarios and `soak_proof.sh` restart harness. Stage 5 HA soak harness (`engine/_dev/s/manual-tests/ha-soak-test.sh`) proves 3-node convergence under sustained restart rotation. Published evidence lives in [`engine/loadtest/BENCHMARKS.md`](../loadtest/BENCHMARKS.md), including the 2026-03-28 2h soak proof pack and Stage 5 HA soak findings. Large-dataset benchmarking (100k docs): deterministic generator (`generate_dataset.mjs`), import throughput (`import_benchmark.sh`), search latency by query type (`search_benchmark.sh`), k6 concurrent load (`benchmark_k6.sh`), and dashboard large-index perf test (`large-index-perf.spec.ts`).
...
| TODO stub cleanup + HA soak hardening | ✅ Done | mar28_pm_3 | Replaced ~601 auto-generated `TODO: Document` stubs with real doc comments across all crates. Added HA soak test script (`engine/_dev/s/manual-tests/ha-soak-test.sh`). Doc-regression tests for server/startup. |
...
k6 test suite in `engine/loadtest/` covering concurrent production traffic patterns. See [`engine/loadtest/README.md`](../loadtest/README.md).
...
**Current focus: interpret the now-complete Stage 3 proof pack honestly.** The major Tier 1 and Tier 2 engineering tracks are complete, and the OSS launch gate is closed. Multi-hour soak capture plus the remaining failure-depth proofs are now landed locally; the next work block is deciding whether to retune or improve prolonged single-node write-overload behavior.
...
3. ~~HA verification~~ — ✅ Done (mar22_3, soak-verified mar28_pm_3). 3-node HA cluster, 2-node replication/analytics fan-out, and S3 snapshot/restore all verified via Docker. Stage 5 HA soak harness (`engine/_dev/s/manual-tests/ha-soak-test.sh`) proves convergence under sustained restart rotation. See [`engine/loadtest/BENCHMARKS.md`](../loadtest/BENCHMARKS.md) for measured findings.
...
| 2026-03-27 | mar27_pm: Launch gate closure + truth-sync | 5-stage launch completion: (1) Staging CI green via gate-closing run `23671792399` on commit `745a059`; (2) truth-synced PRIORITIES.md, ROADMAP.md, HIGHEST_PRIORITY.md to match live launch state; (3) public launch surface audit — README smoke 6/6, doc-link validation green, all live public URLs returning 200; (4) launch proof pack created at `chats/mar27_evening_launch_proof_pack.md`; (5) Algolia compat sprint checklist drafted. |
...
| 2026-03-29 | mar28_pm_3: TODO stub cleanup + HA soak hardening | Replaced ~601 auto-generated `TODO: Document` stubs with real doc comments across all 4 crates (engine/src: 271, flapjack-http: 322, flapjack-server: 3, flapjack-replication: 5). Added HA soak test script (`engine/_dev/s/manual-tests/ha-soak-test.sh`) proving 3-node convergence under sustained restart rotation. Doc-regression tests for `run_graceful_shutdown` and `load_server_config`. Module-level `//!` summaries added to key files. Branch: `mattman/mar28_pm_3_code_hygiene_todo_cleanup_ha_soak`. |
