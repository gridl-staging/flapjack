...
**Last updated: 2026-03-27 (second green wrapper proof at `c7bb8222`; debbie staging sync config hardened)**

- **Backend API:** 197/197 complete (as of 2026-03-13). See the backend parity report (`6_TODO/BACKEND_PARITY_REPORT_2026-03-13.md`) for the full verification details.
...
- **E2E Browser Tests:** 340+ tests across 46 Playwright spec files in total: 42 browser specs (41 specs in `tests/e2e-ui/` [4 smoke + 37 full] plus root-level `tests/result-helpers.spec.ts`) and 4 API-contract specs in `tests/e2e-api/`. Canonical exact-HEAD wrapper evidence is `cd engine && ./s/test --all` **PASS** at commit `aa7dd7db61d7e274cdf946ac6dd7d7435c4dcdf4` (artifact `engine/state/test_summary_all_20260326_191752.md`): all 14 sections passed with exit 0.
- **Tour Video Walkthroughs:** 22/24 specs complete, 2 blocked (vector/chat feature flags). Per-feature tours provide end-to-end workflow proof for core dashboard capabilities.
- **Load & Stress Testing:** k6 suite in `engine/loadtest/` — smoke, search throughput, write throughput, mixed workload, spike, and memory-pressure scenarios. Published baseline artifact: `engine/loadtest/BENCHMARKS.md` (run artifacts in `engine/loadtest/results/20260321T025426Z/`). Large-dataset benchmarking (100k docs): deterministic generator (`generate_dataset.mjs`), import throughput (`import_benchmark.sh`), search latency by query type (`search_benchmark.sh`), k6 concurrent load (`benchmark_k6.sh`), and dashboard large-index perf test (`large-index-perf.spec.ts`).
...
Work tracked in `chats/MAR14_flapjack_ENGINE_checklist.md`.
...
| GL-6 Dashboard feature completeness | ✅ Done | Dashboard route inventory shipped: 22 user-facing routes backed by 21 lazy-loaded page components, plus the not-found catch-all. Tour video acceptance suite covers 22 per-feature specs (01-04, 07-24) with archived MP4 artifacts. Specs 05-06 blocked by vector/chat feature flags. Legacy root walkthrough covers all 21 pages at smoke level. |
...
4. `./s/test --all` green (~20 min full suite) after the dashboard full-suite stability pass. — ✅ Done: exact-HEAD wrapper proof green at commit `aa7dd7db61d7e274cdf946ac6dd7d7435c4dcdf4` (`engine/state/test_summary_all_20260326_191752.md`), all 14 sections passed with exit 0.

## Open-Source Launch Readiness (as of 2026-03-26)
...
| Performance benchmarks published | ✅ Done | mar22_2 | k6 baselines in `loadtest/BENCHMARKS.md` |
| `query-suggestions.spec.ts` full-suite status | ✅ Exact-HEAD wrapper green | mar23_pm_1 + mar24_pm_1 + mar25_pm_12 | Exact-HEAD wrapper proof green at commit `aa7dd7db` (`engine/state/test_summary_all_20260326_191752.md`), all 14 sections passed with exit 0. |
...
| Systemd VPS deployment path | ✅ Done | mar23_pm_3 + mar23_pm_5 + stage_03 + stage_04 | Templates/docs landed in mar23; live VPS end-to-end verification completed on 2026-03-26 and captured in `engine/state/systemd_vps_proof_20260326.md` (Linux ELF at `/opt/flapjack/bin/flapjack`, `EnvironmentFile=/etc/flapjack/env`, `systemctl enable --now`, `/health` + `/health/ready`, manual restart, SIGKILL recovery). |
...
340+ tests across 46 Playwright spec files in total — 42 browser specs (41 specs in `tests/e2e-ui/` [4 smoke + 37 full] plus root-level `tests/result-helpers.spec.ts`) and 4 API-contract specs in `tests/e2e-api/`. Canonical present-tense status is based on exact-HEAD wrapper artifact `engine/state/test_summary_all_20260326_191752.md` at commit `aa7dd7db61d7e274cdf946ac6dd7d7435c4dcdf4`: `cd engine && ./s/test --all` is currently green with all 14 sections passed and exit 0.
...
| Workstream | Scope | Checklist |
|---|---|---|
| A — CRUD & Data Management | Documents, settings, rules, merchandising, API keys, dictionaries, security sources (7 stages, 19/19 checks) | `chats/MAR18_WORKSTREAM_A_CRUD_DATA_MANAGEMENT.md` |
| B — Intelligence & Analytics | Analytics, query suggestions, experiments, personalization, recommendations (5 stages, 18/20 checks) | `chats/MAR18_WORKSTREAM_B_INTELLIGENCE_ANALYTICS.md` |
| C — System, Devtools, Edge Cases | System/metrics/migration reconciliation, devtools, navigation dedup, adversarial search, shared constants (6 stages, 16/16 checks) | `chats/MAR18_WORKSTREAM_C_SYSTEM_DEVTOOLS_EDGE_CASES.md` |
...
| 1 — Core Search | 4/6 done (2 blocked) | 01-overview, 02-search-basic, 03-search-facets, 04-search-synonyms |
...
**Blocked specs:** `05-search-vector-hybrid.spec.ts` (requires `--features vector-search-local` Cargo flag), `06-chat-rag.spec.ts` (requires NeuralSearch mode + AI provider config).

### Load & Stress Testing (k6)

k6 test suite in `engine/loadtest/` covering concurrent production traffic patterns. See `engine/loadtest/README.md`.
...
Added by mar22_2. Deterministic 100k-doc product dataset generator with import throughput, search latency, and concurrent load benchmarks. See `engine/loadtest/BENCHMARKS.md` for results.
...
|---|-----------|--------|-------------|
| PR-1 | `./s/test --all` green | ✅ Done (2026-03-26) | Exact-HEAD wrapper proof green at commit `aa7dd7db61d7e274cdf946ac6dd7d7435c4dcdf4` (`engine/state/test_summary_all_20260326_191752.md`), all 14 sections passed with exit 0. Second independent proof at `c7bb8222` (`engine/state/test_summary_all_20260326_230622.md`) also all 14 sections green. |
...
| PR-3 | Tour video completion (Phases 2–5) | ✅ Done (2026-03-20) | 22/24 specs complete with archived MP4 artifacts. 2 blocked by feature flags (05 vector, 06 chat/RAG). Branch: `mattman/mar19_3_tour_videos_phases_2_5`. |
...
| # | Work Item | Status | Description |
|---|-----------|--------|-------------|
| PR-5 | Accessibility (axe-core + WCAG) | ✅ Done (2026-03-21) | Zero accessibility testing exists. Integrate `@axe-core/playwright` into existing Playwright suite for automated WCAG violation detection (missing labels, broken ARIA, contrast). Enterprise customers may require WCAG 2.1 AA compliance. ~1 session. |
| PR-6 | Deep health check | ✅ Done (2026-03-21) | `/health/ready` ships as an operational readiness probe (`engine/flapjack-http/src/handlers/readiness.rs`, routed in `engine/flapjack-http/src/router.rs`) with canonical 503 failure envelope and 200 on healthy/empty-node states. Bug fixed 2026-03-23: `_usage/` is now excluded from tenant probing in `tenant_dirs.rs` (see `DOC_AUDIT_BLOCKERS.md`). Remaining deep checks still outstanding: S3 accessibility, replication connectivity, explicit index-file readability validation. |
| PR-7 | Latency histograms + performance baseline | ✅ Done (2026-03-21) | Stage 3 shipped request-latency histogram instrumentation (`engine/flapjack-http/src/latency_middleware.rs`) and `/metrics` exposition integration (`engine/flapjack-http/src/handlers/metrics.rs`). Stage 4 published benchmark baseline evidence in `engine/loadtest/BENCHMARKS.md` with run artifacts under `engine/loadtest/results/20260321T025426Z/`; `engine/loadtest/run.sh` now exits with code 99 only for threshold breaches while completing all scenarios. Benchmark figures remain owned by `BENCHMARKS.md`. |
| PR-8 | Error recovery + data durability | ✅ Done (2026-03-21) | Targeted integration tests for: (a) server crash-during-indexing → restart → zero data loss, (b) disk full during batch write → clean failure → recovery when space returns, (c) replication peer disappearance and catch-up reconnection, (d) S3 backup/restore round-trip (snapshot → delete index → restore → verify all records intact — "backup code exists" ≠ "backup works"), (e) multi-tenant isolation under adversarial load (tenant A cannot starve or corrupt tenant B). Circuit breaker and memory pressure middleware exist but are untested under real compound failure conditions. |
...
3. ~~HA verification~~ — ✅ Done (mar22_3). 3-node HA cluster, 2-node replication/analytics fan-out, and S3 snapshot/restore all verified via Docker. Documentation reconciled to match verified proofs.
4. ~~README + docs polish~~ — ✅ Done (mar22_pm_2). Full mechanical code-vs-docs audit across all public-facing documents. Env var completeness verified (OPS_CONFIGURATION.md canonical), feature claims verified against code, quickstart guides reconciled, cross-doc DRY violations fixed, PHASES.md deprecated. 1 code blocker logged (`DOC_AUDIT_BLOCKERS.md`: `/health/ready` unit test vs runtime mismatch). All validation scripts pass.
...
|------|-----------|---------|
| 2026-03-13 | Backend API 197/197 | Full Algolia parity verified. See `6_TODO/BACKEND_PARITY_REPORT_2026-03-13.md`. |
...
| 2026-03-19 | PR-1: `./s/test --all` green | Historical milestone; was later superseded by exact-HEAD wrapper reruns. Final green proof landed 2026-03-26 at commit `a220e66c` (`engine/state/test_summary_all_20260326_004759.md`). |
...
| 2026-03-21 | mar20_2 observability/security (Stage 4 baseline artifact) | Published the loadtest baseline artifact in `engine/loadtest/BENCHMARKS.md` with raw artifacts under `engine/loadtest/results/20260321T025426Z/`; `engine/loadtest/run.sh` now reports threshold-breach runs with exit code 99 after completing all scenarios. |
...
| 2026-03-23 | mar22_pm_1: Full test suite regression gate | Crate-by-crate Rust test validation, fixed taskID alias collisions in index manager (`lifecycle.rs`, `write.rs`, `mod.rs`), fixed 21 dashboard test regressions (unit + e2e), refreshed openapi snapshot fixture, stabilized query-suggestions e2e, and captured wrapper proof artifacts. That run stayed non-green (`a6a12ea1`); launch authority was later superseded by the green proof at commit `a220e66c` (`engine/state/test_summary_all_20260326_004759.md`). Branch: `mattman/mar22_pm_1_full_test_suite_regression_gate`. |
| 2026-03-23 | mar22_pm_2: Documentation accuracy audit | All 6 stages complete. Mechanical code-vs-docs verification across all public-facing documents: env var completeness (OPS_CONFIGURATION.md canonical), feature comparison table verified, quickstart guides reconciled against live contracts, cross-doc DRY violations fixed, PHASES.md deprecated, terminology normalized. Tightened SSL config assertions (`flapjack-ssl/src/config.rs`). 1 code blocker logged in `DOC_AUDIT_BLOCKERS.md` (`/health/ready` unit test vs runtime mismatch). All validation scripts pass. Branch: `mattman/mar22_pm_2_docs_accuracy_audit`. |
| 2026-03-23 | mar22_pm_3: Benchmark validation & integration smoke | New comprehensive API integration smoke test (`engine/tests/integration_smoke.sh`, 513 lines) exercising 13 API path categories. Updated benchmark baseline compilation (`compile_baseline.mjs/sh`). Large-dataset baseline: 100k docs imported in 48.4s, search p95 128ms, all 6 k6 scenarios PASS (Apple M4 Max). Results in `engine/loadtest/BENCHMARKS.md`. Branch: `mattman/mar22_pm_3_benchmark_and_smoke`. |
...
| 2026-03-25 | mar24_pm_1: test hardening at current HEAD | Hardened `query-suggestions.spec.ts` readiness waits, `local-instance-config.ts` parsing/URL handling, Playwright worker override support, socket-churn retries in Rust integration tests, and refreshed the tracked README screenshot PNGs. Targeted validations passed at HEAD; the port-contention issue seen in earlier wrapper runs was resolved in the final green proof at commit `a220e66c` (`engine/state/test_summary_all_20260326_004759.md`). Branch: `mattman/mar24_pm_1_test_suite_green`. |
| 2026-03-25 | stage_04: exact-HEAD wrapper proof refresh | Ran canonical wrapper at commit `23ac8a9e76c90cf2c36c447b812acdcbf0e32d4e` and minted `engine/state/test_output_all_20260325_170929.log` + `engine/state/test_summary_all_20260325_170929.md`. Executed sections `[1]-[5]` passed; first failing executed section was `[6]` Dashboard Playwright smoke (`127.0.0.1:53142` already in use). |
...
| 2026-03-26 | mar25_pm_12: Playwright port hardening + wrapper green proof | Fixed port contention between Playwright smoke and full e2e runs within `./s/test --all`. Added port-release wait step in `engine/_dev/s/test` wrapper and `--wait-port-free` mode in `engine/dashboard/scripts/playwright-webserver.mjs`. New vitest coverage for port-wait logic. JSDoc cleanup in `local-instance-config.ts`. Produced authoritative green exact-HEAD wrapper proof at commit `a220e66c` (`engine/state/test_summary_all_20260326_004759.md`): all 14 sections passed with exit 0. Resolves launch blocker #1 (PR-1). Branch: `mattman/mar25_pm_12_playwright_port_wrapper_proof`. |
| 2026-03-26 | stage_04: VPS systemd proof reconciliation | Published redacted maintained evidence artifact `engine/state/systemd_vps_proof_20260326.md` from the live Stage 3 run/review, updated launch status ledgers to remove stale VPS blocker language, and added the explicit Linux binary prerequisite in `engine/examples/systemd/README.md`. |
...
| 2026-03-26 | mar26_am_2: VPS systemd end-to-end proof | Full end-to-end systemd deployment validated on Ubuntu EC2 (c7i-flex.2xlarge, us-east-1). Service account, unit file, env file, health probes, restart recovery, crash recovery all confirmed. EC2 helper script secret paths fixed (`engine/.secret/` not `.secret/`). Resolves launch blocker #2. Evidence: `engine/state/systemd_vps_proof_20260326.md`. Branch: `mattman/mar26_am_2_vps_systemd_proof`. |
...
| 2026-03-27 | mar26_pm_4: Dev repo test suite + Docker build | Second independent full test suite proof at HEAD. All 14 sections green (`engine/state/test_summary_all_20260326_230622.md`). Docker build verified with container health + search smoke test. Added `search_compat` shim unit + integration tests. Branch: `mattman/mar26_pm_4_dev_repo_test_suite_and_docker`. |
| 2026-03-27 | mar26_pm_5: Debbie staging sync config hardening | Fixed identity values from legacy `gridlhq*` to correct `gridl-dev`/`gridl-staging`/`gridl-hq`. Added `ROADMAP.md`, `engine/docs2/` strategy docs, `engine/examples/`, `integrations/laravel-scout/`, CI shell scripts to sync whitelist. Sanitized `FEATURES.md` and `TESTING.md` to remove private dashboard path references for public staging. Executed real debbie sync to staging clone. Stage 4 (staging push + CI) deferred. Branch: `mattman/mar26_pm_5_debbie_sync_staging_ci`. |
