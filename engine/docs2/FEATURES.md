...
**Last updated: 2026-03-27 (OSS launch gate closed on staging run `23671792399`; public loadtest baseline docs synced)**
...
- **Load & Stress Testing:** k6 suite in `engine/loadtest/` — smoke, search throughput, write throughput, mixed workload, spike, and memory-pressure scenarios. Published baseline artifact: [`engine/loadtest/BENCHMARKS.md`](../loadtest/BENCHMARKS.md). Large-dataset benchmarking (100k docs): deterministic generator (`generate_dataset.mjs`), import throughput (`import_benchmark.sh`), search latency by query type (`search_benchmark.sh`), k6 concurrent load (`benchmark_k6.sh`), and dashboard large-index perf test (`large-index-perf.spec.ts`).
