...
pub(super) fn execute_search_query(
    ...
    let total = resolve_total_hits(&query_totals, all_results.len(), effective_limit);
...
struct ZeroLimitSearchContext<'a> {
    facets: Option<&'a [crate::types::FacetRequest]>,
    max_values_per_facet: Option<usize>,
    facet_cache_key: Option<&'a String>,
