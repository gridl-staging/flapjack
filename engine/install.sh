...
#   curl -fsSL https://staging.flapjack.foo | sh -s -- v1.0.10   # pinned version
#
...
REPO="${FLAPJACK_REPO:-gridl-staging/flapjack}"
...
get_version() {
  ...
  # Parse version from CLI args (e.g., `| sh -s -- v1.0.10`)
  if [ -n "${1:-}" ]; then
    version="$1"
    return
  fi
  ...
  if [ -z "$version" ]; then
    ...
    error "You can also specify a version: curl ... | sh -s -- v1.0.10"
    exit 1
  fi
...
download_release_asset() {
  ...
  if [ -n "${GITHUB_TOKEN:-}" ]; then
    ...
    if [ -n "$asset_url" ]; then
      ...
      if [ "$downloader" = "curl" ]; then
        if curl -fsSL -H "Authorization: token ${GITHUB_TOKEN}" -H "Accept: application/octet-stream" -o "$output" "$asset_url" 2>/dev/null; then
          return 0
        fi
      else
        if wget -q --header="Authorization: token ${GITHUB_TOKEN}" --header="Accept: application/octet-stream" -O "$output" "$asset_url" 2>/dev/null; then
          return 0
        fi
      fi
      # API download failed — fall through to direct URL
    fi
  fi

  # Fallback: direct URL (works for public repos without token)
