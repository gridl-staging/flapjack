// Regression guard: `flapjack-http`'s default features must be empty so the
// baseline build does not transitively pull `vector-search-local`'s heavy
// embedding chain. See
...
#[test]
fn flapjack_http_default_features_omit_vector_search_local() {
    let meta = cargo_metadata_json();
    let defaults = default_features_for(&meta, "flapjack-http");
    assert!(
        !defaults.iter().any(|f| f == "vector-search-local"),
        "flapjack-http default features must not contain `vector-search-local`; got {defaults:?}"
    );
    let expected: Vec<String> = Vec::new();
    assert_eq!(
        defaults, expected,
        "flapjack-http default features must be exactly the empty set"
    );
}
