...
if [ "$status" -eq 0 ]; then
  echo "Expected _dev/s/test --dashboard to stop at the stubbed dashboard unit command after stale-node_modules validation"
  exit 1
fi
...
echo "PASS: _dev/s/test revalidates and rehydrates stale real dashboard/node_modules directories"
