...
if [ "$status" -eq 0 ]; then
  echo "Expected s/test --sdk to stop at the stubbed JS entrypoint on a valid cache-hit reuse path"
  exit 1
fi
...
echo "PASS: s/test reuses valid sdk_test/node_modules cache and continues into SDK tests"
