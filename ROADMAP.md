...
| — | Nightly CI + sync hygiene | ✅ Done (2026-03-31) | Restored nightly Rust CI parity with the dashboard dist stub, added `CHANGELOG.md` / `CONTRIBUTING.md` / `SECURITY.md` to `.debbie.toml`, and clarified README vector/hybrid platform caveats. This landed after wave 3 and now needs the next debbie sync to reach public clones. |
| — | Operator runbook hardening | ✅ Done (2026-03-31) | `engine/docs2/3_IMPLEMENTATION/OPERATIONS.md` now carries proof-backed startup/readiness/replication/admin-key/snapshot failure runbooks plus corrected `reset-admin-key` syntax and tighter proof citations. This landed after wave 3 and now needs the next debbie sync to reach public clones. |
| — | Security baseline docs + test coverage | ✅ Done (2026-03-31) | `SECURITY_BASELINE.md` and `4_EVIDENCE/SECURITY_BASELINE_AUDIT.md` now document the shipped HTTP hardening surface (CORS, trusted proxies, per-key rate limiting, body limits) with focused proof references, and `startup_catchup.rs` now warns-and-continues on write-queue timeout outside strict bootstrap. This landed after wave 3 and now needs the next debbie sync to reach public clones. |
| — | Security baseline follow-through | ✅ Done (2026-03-31) | Closed the two scoped HTTP-hardening proof gaps by adding invalid-key non-consumption and `FLAPJACK_MAX_BODY_MB` `413` tests, extracting `max_body_mb_from_value`, aligning the security doc/audit, refreshing the committed OpenAPI export, and blocking symlinked sync destinations in helper scripts. This landed after wave 3 and now needs the next debbie sync to reach public clones. |
| — | Runbook parity + admin-key truth sync | ✅ Done (2026-03-31) | Standardized `flapjack --data-dir <path> reset-admin-key` across startup output, dashboard auth help, `AUTH_DESIGN.md`, and `OPERATIONS.md`, including quoting for spaced data-dir paths. This landed after wave 3 and now needs the next debbie sync to reach public clones. |
| — | Experiment handler merge guardrails | ✅ Done (2026-03-31) | `/2/abtests/{id}/results` now uses the shared resolver seam, targeted results-endpoint proofs cover store-unavailable and numeric/UUID resolution, and experiment OpenAPI docs now declare resolver-driven `500` responses consistently. This landed after wave 3 and now needs the next debbie sync to reach public clones. |
...
| — | Next public sync wave | In progress | Staging/prod remain at wave 3 (`6166055` / `b7841a0`). Dev `main` is now ahead with the Mar 31 pm1-pm6 stack plus Apr 8 and Apr 15 work and should publish that truth next. |
